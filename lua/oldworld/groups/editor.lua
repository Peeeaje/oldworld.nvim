...
return {
  ...
  DiffAdd = { bg = p.dark_green },                       -- diff mode: Added line
  DiffChange = { bg = p.dark_yellow, underline = true }, -- diff mode: Changed line
  DiffDelete = { bg = p.dark_red, },                     -- diff mode: Deleted line
  DiffText = { bg = p.dark_yellow, },                    -- diff mode: Changed text within a changed line
  EndOfBuffer = { fg = p.bg },
  ...
  LineNr = { fg = p.subtext4 },                          -- Line number for " =number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
  ...
  SpellBad = { fg = p.orange, undercurl = true },        -- Word that is not recognized by the spellchecker. This will be combined with the highlighting used otherwise.
